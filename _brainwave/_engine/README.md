...
`_dashboard.html` remains the single self-contained, `file://`-compatible user artifact. It is generated by `dashboard_renderer.js` from `dashboard/shell.html` and the lexically ordered fragments beneath `dashboard/styles/` and `dashboard/scripts/`.

Edit those source files rather than `_dashboard.html`. Numeric filename prefixes define assembly order, so JavaScript fragments share one private application scope without requiring browser modules, external assets, a package installation, or a build server. Keep lifecycle and implementation-state derivation in `brainwave_runner.js`; dashboard source should only present the manifest it receives.
