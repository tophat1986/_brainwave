...
function bridgeBlock() {
  return [
    ...
    "- At `brainwave_documentation_complete`, use ambient delivery alignment: identify the DNA blocks directly affected by a task, read their owning documents before editing, and reconcile their status and evidence before claiming completion.",
    ...
    "- During downstream implementation, update only the status and concise current evidence of DNA blocks directly affected by the work; do not create a separate implementation log.",
    "- Never silently rewrite accepted DNA direction to match implementation. Obtain user approval for behavioural changes, supersede the affected block while retaining a compact tombstone, and reopen the appropriate lifecycle stage when the North Star, relevant domains, or document scope changes.",
