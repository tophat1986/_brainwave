...
<!-- _brainwave:project-bridge:start -->
## _brainwave
...
- When reference material is relevant, read `_brainwave/_reference_library_guide.md` and use the indexed project-owned `_brainwave/_references/` library. Search textual metadata first and inspect only shortlisted media; references remain contextual inputs unless accepted into the North Star or a DNA block.
...
- Follow the active slice's sealed assurance gate and inherited minimum levels. Resolve applicable UI harness decisions, prepare its bounded QA packet, use the required reviewer mode, remediate stable findings, and recheck at the current revision before closure; never treat implementation evidence alone as QA approval.
- Follow `_brainwave/_settings.yaml` `implementation_progress_updates` only during Deliver the implementation: `silent` gives no routine updates, `track` reports when every slice in a track is verified, and `slice` reports each closed slice. This controls update frequency only; decision authority and continuation follow `implementation_mode`. Once that mode and the exact plan are approved, continue across eligible work without treating updates as permission requests. Pause when implementation authority is stale or invalid, for required safety authorization, or when required input, approval, access, an unresolved blocker, or an external gate leaves no other safe eligible work.
